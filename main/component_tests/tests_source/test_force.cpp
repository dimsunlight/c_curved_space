...
std::pair<double,Vector_3> calcTangentandDistance (Triangle_mesh mesh, Point_3 source, Point_3 target) {
  ...
  clock_t beforeTree = clock();
  AABB_tree tree;
  shortest_paths.build_aabb_tree(tree);
  std::cout << "Building a tree took " << ((double) (clock() - beforeTree)) / CLOCKS_PER_SEC << " seconds." << std::endl;

  clock_t beforeLocate = clock();  
  Face_location target_loc = shortest_paths.locate<AABB_face_graph_traits>(target,tree);
  std::vector<Point_3> points;
  std::cout << "Running locate took  " << ((double) (clock() - beforeLocate)) / CLOCKS_PER_SEC << " seconds." << std::endl;
  

  clock_t beforePath = clock(); 
  shortest_paths.shortest_path_points_to_source_points(target_loc.first, target_loc.second, std::back_inserter(points));
  std::cout << "Finding a path took  " << ((double) (clock() - beforePath)) / CLOCKS_PER_SEC << " seconds." << std::endl;

  
  double distance;
  Vector_3 tangent;
  
  //tangent is a constant-time operation and a pretty brief one at that -- feels unlikely that even needs to be timed 
