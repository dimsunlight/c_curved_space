...
Point_3 find_intersection(Vector_3 source_bary, Vector_3 query_bary) {
  ...
  for(double val: intersection_values) {
    if (val < toIntersect) toIntersect = val;
  ...
  if (toIntersect < 1) { 
    ...
    min_intersection = Point_3(source_bary.x(),source_bary.y(),source_bary.z());
...
Vector_3  move(Point_3 source, Point_3 target, std::vector<Point_3> faceVertices ) {
  ...
  find_intersection(source_bary_vector,query_bary_vector);
...
int main(int argc, char* argv[]) {
 ...
 move(movedOntoMesh,movedOntoMesh+forceDisplacement, faceVertices);

 std::cout << "r3 displacement magnitude is " << vector_magnitude(forceDisplacement) << std::endl; 
 return 0;
}
