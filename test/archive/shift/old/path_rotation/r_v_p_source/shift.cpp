...
Point_3 shift(Triangle_mesh mesh, const Point_3 pos, const Vector_3 move) {
  ...
  Point_3 oldTarget; //target before we rotate/project down the move vector
  std::vector<Point_3> forRotation;
  Vector_3 sourceFaceNormal;
  ...
  while(intersection){
    ...
    std::cout << "source face vertices" << std::endl;
    for (Point_3 vertex: vertexList) std::cout << vertex << std::endl;
    ...
    sourceFaceNormal = PMP::compute_face_normal(currentSourceFace,mesh);
    Vector_3 vector_to_intersection = Vector_3(source_point, intersection_point);
    lengthToSharedElement = vectorMagnitude(vector_to_intersection); //how far we've traveled
    ...
    oldTarget = intersection_point + current_move;
    ...
    forRotation = {oldTarget}; //setting up a vector for rotateAboutAxis  
    double rotationAngle = angleBetween(currentSourceFace,currentTargetFace, mesh); //
    oldTarget = rotateAboutAxis(forRotation,sharedEdge,rotationAngle)[0];

    auto alt_current_move = Vector_3(source_point, oldTarget); 

    std::cout << "projection-based current_move: " << current_move << std::endl;
    std::cout << "rotation-based current_move  : " << alt_current_move << std::endl;
