...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < 100; i++) {
    ...
    //if i used one of the n_sample functions, we would accidentally resample point1
    //first every time; so i can just do it the slow way below
    targetcount = i; 
    for (int j = 1; j <= i; j++) {
    ...
    if (ftime_file.is_open()){
      ftime_file << i << ", " <<  f_time.count();
      ftime_file << "\n";
    }
  }
