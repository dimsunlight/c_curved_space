...
int main(int argc, char* argv[]) {
  std::string filename = "sp_500pts.csv"; 
  std::ofstream sp_100sample(filename);

  std::vector<Point_3> samplePoints = n_sinplane_sample_points(500, 3109874); 
  
  for (Point_3 p: samplePoints) {
    sp_100sample << p.x() << ", " << p.y() << ", " << p.z() << std::endl; 
  }
