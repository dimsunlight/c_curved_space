...
int main(int argc, char* argv[]) {
  ...
  std::string timefile_name = "time_to_sequence_tree.csv";
  ...
  for (int i = 1; i <= maxN; i++) {
    ...
    for (int j = 1; j <= targetcount; j++) {
      points.push_back(torus_sample(1,3,gen));
    }    

    time_to_tree = time_big_sequence_tree(mesh, points);

    if (timefile.is_open()){
      timefile << targetcount << ", " <<  time_to_tree;
      timefile << "\n";
    }
  }
